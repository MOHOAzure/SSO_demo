...
async def lifespan(app: FastAPI):
    ...
    load_keys()
    cache_idp_jwks()
    logger.info("Client1 started", extra={"event": "startup", "service": "client1"})
    yield
    # Shutdown
    logger.info("Client1 shutdown", extra={"event": "shutdown", "service": "client1"})
...
class PrometheusMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        # Do not record HTTP metrics here, as we focus on business metrics
        return response

app = FastAPI(title="Client 1 - Stage 3 (Monitoring)", lifespan=lifespan)
...
def create_session_cookie(data: Dict) -> str:
    ...
    payload = {
        **data,
        "iss": CLIENT_BASE_URL,
        "exp": datetime.now(timezone.utc) + timedelta(minutes=30), # 30 minute lifetime
        "iat": datetime.now(timezone.utc)
    }
    return jwt.encode(payload, CLIENT_PRIVATE_KEY, algorithm="RS256")

def verify_session_cookie(cookie_value: str) -> Optional[Dict]:
    """Validate signed JWT session cookie"""
    if not CLIENT_PUBLIC_KEY:
...
async def verify_session(request: Request) -> Optional[Dict]:
    """FastAPI dependency to validate session and return user info"""
    session_cookie = request.cookies.get("client_session")
    if not session_cookie:
        session_verification_failures.labels(reason='missing_cookie', client='client1').inc()
        return None
    
    session_data = verify_session_cookie(session_cookie)
    if not session_data:
        session_verification_failures.labels(reason='verification_failed', client='client1').inc()
        return None
...
# Cache IdP's JWKS on application startup
IDP_JWKS = {}
# @app.on_event("startup") # REMOVED
def cache_idp_jwks():
    global IDP_JWKS
...
async def home(request: Request, silent_auth_attempted: Optional[str] = None):
    ...
    # If no local session and silent login not attempted, try silent login (backend handled)
    ...
    # User is logged in or silent login attempted, show home page
    return templates.TemplateResponse("client_home.html", {
        "request": request, "user": user, "client_name": "Client 1", "client_url": CLIENT_BASE_URL
    })
...
async def callback(request: Request, code: Optional[str] = None, state: Optional[str] = None, error: Optional[str] = None):
    ...
    if error:
        callback_total.labels(status='failure', error_type=error, client='client1').inc()
        logger.warning(
            "Callback failed - error from IdP",
            extra={
    ...
    if not code or not state:
        callback_total.labels(status='failure', error_type='missing_parameters', client='client1').inc()
        logger.error(
    ...
    if not state_cookie_value:
        callback_total.labels(status='failure', error_type='missing_state_cookie', client='client1').inc()
        logger.error(
    ...
    if not state_data:
        callback_total.labels(status='failure', error_type='state_cookie_verification_failed', client='client1').inc()
        logger.error(
    ...
    if state != state_data.get("state"):
        callback_total.labels(status='failure', error_type='state_mismatch', client='client1').inc()
        logger.error(
    ...
    try:
        ...
        callback_total.labels(status='success', error_type='none', client='client1').inc()
        logger.info(
            "Callback successful - user logged in",
            extra={
        ...
        callback_total.labels(status='failure', error_type='token_exchange_failed', client='client1').inc()
        logger.error(
            "Callback failed - token exchange error",
            extra={
                "event": "callback_failure",
                "service": "client1",
